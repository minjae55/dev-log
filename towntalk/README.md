...
> Spring 백엔드의 인증·인가와 데이터 조회 구조를 직접 구현하고 검증하기 위한 개인 학습형 MVP 프로젝트입니다.

## Overview

지역 기반 게시판을 중심으로 로그인, 게시글, 댓글, 좋아요와 마이페이지까지 웹 서비스의 핵심 흐름을 구현했습니다. 완성된 서비스 출시보다 Spring Security, JWT, Redis, JPA와 QueryDSL을 실제 기능에 적용하며 동작 원리와 문제 해결 과정을 익히는 데 목적이 있습니다.

## Project Status

핵심 사용자 흐름은 연결했으며, 테스트와 예외 처리, 동시성 및 조회 성능은 이후 계속 보강할 예정입니다.
...
- JWT Access Token과 Refresh Token을 이용한 로그인 유지 및 재발급
- HttpOnly Cookie와 Redis를 이용한 Refresh Token 관리
- 지역 기반 게시글과 댓글 작성·조회·수정·삭제
- 인증 사용자와 작성자를 비교한 수정·삭제 권한 검사
- 게시글 좋아요 토글과 집계 조회
- 마이페이지 프로필, 통계와 활동 내역 조회

## Key Problem Solving

- [`401` 인증 오류처럼 보인 DB 제약조건 문제 추적](<daily/2026-05-31 글쓰기 오류 추적.md>)
- [좋아요 저장 데이터와 화면 집계 기준 불일치 해결](<daily/2026-06-07 좋아요와 집계 조회.md>)
- [QueryDSL Projection 생성자 타입 불일치 해결](<daily/2026-06-07 좋아요와 집계 조회.md>)
