...
로컬 개발 환경을 팀 공용 인프라와 완전히 분리하고(Docker MySQL/Redis), Java Faker로 대량 시드 데이터를 구축했습니다. k6로 협의 목록 조회, 주문 생성 두 시나리오의 부하테스트를 완료해 목표 기준치를 통과했고, 남은 결제 confirm 시나리오의 테스트 방식을 검토 중입니다. 아래는 진행 중인 작업과 아직 막혀있는 부분을 구분해 기록합니다.
...
인증은 이메일/비밀번호 로그인 후 발급되는 `accessToken` 쿠키(HttpOnly) 기반이며, `Authorization` 헤더는 사용하지 않습니다. 처음엔 k6가 가상유저(VU) 단위로 쿠키를 자동 유지해줄 거라 가정했는데, 실제로는 반복(iteration) 사이에 안정적으로 유지되지 않는 문제를 겪었습니다. 그래서 로그인 응답에서 쿠키 값을 직접 꺼내 매 요청에 명시적으로 실어 보내는 방식으로 바꿨습니다. (자세한 내용: [블로그](https://min-soon.tistory.com/112))
