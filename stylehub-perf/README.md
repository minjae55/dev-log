...
로컬 개발 환경을 팀 공용 인프라와 완전히 분리하고(Docker MySQL/Redis), Java Faker로 대량 시드 데이터를 구축했습니다. k6로 협의 목록 조회, 주문 생성 두 시나리오의 부하테스트를 완료해 목표 기준치를 통과했고, 주문 생성 코드를 살펴보던 중 재고가 어디서도 차감되지 않아 무제한으로 초과 판매되는 버그를 발견해 원자적 조건부 UPDATE로 수정하고 재검증까지 마쳤습니다. 남은 결제 confirm 시나리오의 테스트 방식을 검토 중입니다. 아래는 진행 중인 작업과 아직 막혀있는 부분을 구분해 기록합니다.
